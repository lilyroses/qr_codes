...
def combine_like_terms(terms:list[tuple[int]]):
  ...
  To combine like terms, find all terms that have the
  same x exponent.
  Each tuple in `terms` has 2 integers. The first integer
  represents the `a` variable's exponent. The second
  integer represents the `x` variable's exponent.

  For example, if:

      terms = [(0,3), (25,2), (1,1), (2,2), (27,1), (3,0)]

  There are 2 like terms with an x exponent of 2
  (the second number in the tuple):
  ...
  The next step is to f_ind the integer representation of
  each `a` variable's exponent for the like terms (the
  first number in each table), using the  LOG_TABLE:
  ...
  """
  # Step 1: find all terms with the same x exponent
  x_exps = [x for a, x in terms]
  x_exp_counts = {x_exp: x_exps.count(x_exp) if x_exps.count(x_exp) > 1 for x_exp in x_exps}
  # t a list of the final terms of the terms that don't have any like terms
  final_terms = [(a,x) for (a,x) in terms if x not in x_counts]

  for x_exp in x_counts:
    terms_to_combine = [a for a, x in new_terms if x == x_exp]
